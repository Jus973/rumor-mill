 ...
 * before lock (it cannot read them at purchase time — reputation is the only signal it
 * has), decrypts on key delivery, verifies each payload against its on-chain commitment,
 * and pools the survivors with the public prior into one START/BENCH call per slot.
 */
...
import { Outcome } from '../lib/enums.js';
...
async function run() {
  ...
  await runAgent({
    ...
    banner: [
      ...
      `strategy  bounty every QUESTIONABLE/DOUBTFUL slot; buy the top ${K} sealed claims per slot`,
      `          ranked by ledger reputation, then ensemble with the public prior`,
      `pays      ${fmt(REVEAL_FEE)} reveal fee + ${fmt(CONTINGENT)} escrow per claim`,
    ],
    step: async ({ state, fixture, chainNow }) => {
      ...
      if (!posted) {
        sub(`${NAME}: posting bounties on the uncertain slots`);
        for (const g of fixture.games) {
          for (const p of g.players) {
            info(`  ${p.name.padEnd(22)} official report: ${p.prior.tag}/${p.prior.practice} → uncertain, bountying`);
          }
        }
      ...
      if (!locked) {
        // Buy the best sealed claims available. Content is unreadable at this point.
      ...
      if (!done) {
        ...
        const mine = [...state.claims.values()].filter(
          (c) => c.buyer?.toLowerCase() === me.address.toLowerCase() && inRun(state, fixture, c.bountyId),
        );
        if (mine.length > 0 && mine.every((c) => c.settled || c.slashed || c.refunded)) {
          sub(`${NAME}: settled — collecting refunds on wrong and unrevealed claims`);
